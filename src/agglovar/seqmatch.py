...
from .align import score as align_score
from .align import op as align_op
...
class MatchScoreModel:
    score_model: align_score.AffineScoreModel
    map_limit: int|None
    jaccard_kmer: int
    rotate_min: int
    ...
    def __init__(self,
        match=align_score.AFFINE_SCORE_MATCH,
        mismatch=align_score.AFFINE_SCORE_MISMATCH,
        affine_gap=align_score.AFFINE_SCORE_GAP,
        map_limit=2000,
        jaccard_kmer=9,
        rotate_min=3
    ):

        self.score_model = align_score.AffineScoreModel(
            match=match,
            mismatch=mismatch,
            affine_gap=affine_gap
        )
        ...
        if map_limit is not None:
            try:
                ...
                raise ValueError(f'ScoreAligner(): Parameter map_limit must be an integer or None (no limit): {map_limit}')
        else:
            self.map_limit = None

        try:
            self.jaccard_kmer = int(jaccard_kmer)
        except ValueError:
            raise ValueError(f'ScoreAligner(): Parameter jaccard_kmer must be an integer: {jaccard_kmer}')

        try:
            self.rotate_min = int(rotate_min)
        except ValueError:
            raise ValueError(f'ScoreAligner(): Parameter rotate_min must be an integer: {rotate_min}')
    ...
    def score_align_native(self,
        ...
        if len(self.score_model.score_affine_gap) != 1:
            raise RuntimeError(f'Legacy align only support single-affine gap models, found {len(self.score_model.score_affine_gap)} affine segments')
        ...
        trace_matrix = [ScoreTraceNode() for _ in range(len_a)]
        trace_matrix_last = [ScoreTraceNode() for _ in range(len_a)]
        ...
        for j in range(1, len_b):
            ...
            for i in range(1, len_a):
                ...
                score_gap_sub = trace_matrix_last[i].score + (
                    score_gap_extend if trace_matrix_last[i].op_code == OP_GAP_SUB else gap_1bp
                )
                ...
                score_gap_qry = trace_matrix[i - 1].score + (
                    score_gap_extend if trace_matrix[i - 1].op_code == OP_GAP_QRY else gap_1bp
                )
    ...
    def match_prop(self,
        ...
        """
        Get the alignment score proportion over the max possible score between two sequences. To follow tandem
...
def get_kmer_count(
    ...
    :return: Counter with k-mers as keys and counts as values.
    """
