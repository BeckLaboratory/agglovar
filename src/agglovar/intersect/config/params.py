"""
Intersect configuration objects are populated after lexing and parsing the parameter
...
An IntersectSpec object is a fully configured object with the specification type
...
base IntersectSpec class carries out most of the operations to check and set
...
type is a subclass of IntersectSpec and has a hard-coded parameter list. Constructing
a IntersectSpec object is done by determining the correct IntersectSpec subclass based
...
RESERVED_PARAM_NAMES = {
    ...
    'align_param',       # Alignment parameters used to initialize the aligner
...
class IntersectSpec(object):
    ...
    def __init__(self, spec_type, arg_list, param_spec_list, allow_matcher=True):
        ...
        self.match_score_model = None  # Configured match score model
        ...
        for index in range(len(arg_list)):
            ...
            # Check formatting
            if param_tuple is None:
                raise RuntimeError(f'Specification type {self.spec_type}: Cannot check parameters: None')
            ...
            if param_tuple[0] == 'match':
                ...
                self.set_matcher(IntersectSpecMatch(param_tuple[1]))
    ...
    def __repr__(self, show_matcher=True):
        repr_str = f'IntersectSpec({self.spec_type}:'
...
class IntersectSpecSzro(IntersectSpec):
    """
...
class IntersectSpecDistance(IntersectSpec):
    """
...
class IntersectSpecRo(IntersectSpec):
    """
...
class IntersectSpecExact(IntersectSpec):
    """
...
class IntersectSpecTruvari(IntersectSpec):
    """
    Param set: Truvari
