...
RESERVED_PARAM_NAMES = {
    'spec_type', 'arg_list', 'intersect_strategy', 'param_spec_list', 'allow_matcher',
}
...
class IntersectStage(object, metaclass=ABCMeta):
    ...
    :ivar arg_list: List of actual arguments used to create this spec. Each element is a tuple of (type, value, name)
        extracted from the parameter specification string during parsing.
    :ivar intersect_strategy: Link to the IntersectStrategy object creating this step. This is used to query the intersect
        configuration for global parameters and a global matcher.
    :ivar param_spec_list: List of parameter specifications describing the parameters this specification can take,
    ...
    def __init__(
        ...
        :param arg_list: List of arguments used to configure this parameter set.
        :param intersect_strategy: Link to the IntersectStrategy object creating this step.
        :param param_spec_list: List of parameter specifications.
        ...
        for index, param_spec in enumerate(param_spec_list):
            ...
            assert param_spec.name not in self.__dict__.keys(), \
                f'Specification type %s: Param spec name clashed with a name already defined in the parameter set: %s' % (spec_type, param_spec.name)
        ...
        for index in range(len(arg_list)):
            ...
            if len(param_tuple) != 3:
                raise ValueError(f'Specification type {self.spec_type}: Expected 3 parameters in param_tuple: Found {len(param_tuple)}')

            # Check for matcher
            if matcher_state:
                raise ValueError(f'Specification type {self.spec_type}: Parameter at position {index + 1} follows a matcher (matcher must be the last item in a parameter list)')

            if param_tuple[0] == 'match':
                if not allow_match_stage:
                    raise ValueError(f'Specification type {self.spec_type}: Found matcher at position {index + 1}: No matcher is allowed for this specification type')
            ...
            if param_tuple[2] is None:
                if named_state:
                    raise ValueError(f'Specification type {self.spec_type}: Unnamed value {param_tuple[0]} at position {index + 1} follows a named value: Positional parameters must come before named parameters')

                param_index = index

                if param_index >= len(param_spec_list):
                    raise ValueError(f'Specification type {self.spec_type}: Positional parameter {index + 1} is out of range: Max {len(param_spec_list)}')

            else:
                param_index = param_spec_index.get(param_tuple[2], None)

                if param_index is None:
                    raise ValueError(f'Specification type {self.spec_type}: Unknown named parameter: {param_tuple[2]}')

                assert param_index < len(param_spec_list), \
                    f'Specification type %s: Named parameter at position %d (%s) returns a parameter specification index that is out of range: %d: Max %d' % (
    ...
    def set_match_stage(
            self,
            match_stage: IntersectStageMatch|None=None
    ) -> None:
        """
        ...
        if not self.allow_match_stage:
            raise ValueError(f'Specification type {self.spec_type}: Match stage is not allowed for this specification type')

        self.match_stage = match_stage

    def get_matcher(self) -> seqmatch.MatchScoreModel|None:
        """
        Get the matcher object for this intersect spec. If a matcher is set, for this spec, it is returned. If not,
        a global matcher is returned if set. If neither, None is returned.

        :return: Matcher object or None.
        """
...
class IntersectStageRo(IntersectStage):
    ...
    def __init__(self, arg_list, intersect_strategy):
        super().__init__(
            spec_type='ro',
            arg_list=arg_list,
            ...
            param_spec_list=[
                IntersectParamSpec(
                    ...
                    min_val=(0.0, False), max_val=(1.0, True)
                ),
                IntersectParamSpec(
...
class IntersectStageDistance(IntersectStage):
    ...
    `min(abs(pos_l - pos_r), abs(end_l - end_r))`
    
    At least one of `dist` or `szdist` must be set.
    
    :ivar dist: Distance threshold.
    :ivar szdist: Distance threshold for SVs with size less than `szdist`.
    :ivar szro: RO threshold for SVs with size less than `szdist`.
    :ivar alt: Include ALT calls in intersect.
    :ivar ref: Include REF calls in intersect.
    """
...
class IntersectStageExact(IntersectStage):
    ...
    :ivar alt: Match on ALT.
    :ivar ref: Match on REF.
    """
...
class IntersectStageMatch(IntersectStage):
    ...
    limit: int
    ksize: int
    matcher: seqmatch.MatchScoreModel|None
    ...
    :ivar gap: List of gap scores. Must contain an even number of values where each pair of values is a gap-open
        and gap-extend score. This is a flat list, not a list of tuples, although pairs of values are packaged
        as tuples when creating the match object.
    ...
    def __init__(self, arg_list, intersect_strategy):
        ...
        super().__init__(
            ...
            param_spec_list=[
                ...
                IntersectParamSpec(
                    name='match', default=align.score.AFFINE_SCORE_MATCH, val_type='num'
                ),
                IntersectParamSpec(
                    name='mismatch', default=align.score.AFFINE_SCORE_MISMATCH, val_type='num'
                ),
                IntersectParamSpec(
                    name='gap', default=[val for vals in align.score.AFFINE_SCORE_GAP for val in vals], val_type='list',
                    list_type='num'
                ),
                IntersectParamSpec(
        ...
        self.matcher = seqmatch.MatchScoreModel(
            match=self.match,
...
class IntersectParamSpec(object):
    """
    Parameter specification. Objects of this type are used to define legal parameters, their types, and ranges of
    values. This class provides functions for checking and enforcing parameter values so that configuration objects
    can use them safely, and error checking code is largely consolidated here.

    If the value is a list, then each element inside the list is checked against the list type. If successful, then
    each element inside the list is checked against the minimum and maximum values, if set.

    :ivar val_type: Type of value (e.g. "num", "int", "list").
    :ivar default: Default value if not specified.
    :ivar name: Parameter name.
    :ivar min_val: Minimum value.
    :ivar min_inclusive: Minimum value is inclusive.
    :ivar max_val: Maximum value.
    :ivar max_inclusive: Maximum value is inclusive.
    :ivar unlimited_val: If "unlimited" is given, then set this value.
    :ivar list_type: If not None, then this is a list of values and each element in the list must match this type. If
        None, then the value is not a list.
    """
    ...
    def __init__(
        ...
        default: Any,
        val_type: str,
        min_val: tuple[Any, bool]=None,
        max_val: tuple[Any, bool]=None,
        unlimited_val: Any=None,
        ...
        if min_val is not None:
            if not isinstance(min_val, tuple) or len(min_val) != 2 or not isinstance(min_val[1], bool):
                raise RuntimeError(f'min_val: Expected a tuple of two values: {min_val} (type "{type(min_val)}")')

            self.min_val, self.min_inclusive = min_val

        if max_val is not None:
            if not isinstance(max_val, tuple) or len(max_val) != 2 or not isinstance(max_val[1], bool):
                raise RuntimeError(f'max_val: Expected a tuple of two values: {max_val} (type "{type(max_val)}")')

            self.max_val, self.max_inclusive = max_val
    ...
    def check(self,
        ...
        :return: The value of the object after checking (i.e. the value extracted from `param_tuple`).
        """
        ...
        if param_type not in TYPE_MATCHER[self.val_type]:
            assert param_type in TYPE_MATCHER.keys(), \
                f'Unrecognized type for parameter "%s = %s": %s' % (param_name, param_value, param_type)
        ...
        if param_type == 'unlimited':
            if not self.allow_unlimited:
                raise ValueError(f'Illegal value for "{param_name} = unlimited": Unlimited values are not allowed for this parameter in this intersect specification')
