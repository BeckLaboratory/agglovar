...
class PairwiseOverlap(PairwiseJoin):
    ...
    def join_iter(
        ...
        if self.offset_max == 0 or self.chunk_size == 0:
            return self.join_iter_notchunked(df_a, df_b)

        return self.join_iter_chunked(df_a, df_b)

    def join_iter_chunked(
            self,
            df_a: pl.DataFrame | pl.LazyFrame,
            df_b: pl.DataFrame | pl.LazyFrame,
    ) -> Iterator[pl.LazyFrame]:
        ...
        # Get join function
        join_func: Callable[[pl.LazyFrame, pl.LazyFrame], pl.LazyFrame] = (
            self._join_equi if self.offset_max == 0 else self._join_non_equi
        )
    ...
    #
    #     # Get join function
    #     join_func = self._join_equi if self.offset_max == 0 else self._join_non_equi
    #
    #     # Prepare tables
    #     df_a, df_b = self.prepare_tables(df_a, df_b, warn_on_reserved=True)
    #
    ...
    def _join_equi(
        ...
        """Join on exact positions (pos and end).
        ...
        df_join = (
            ...
            .join(
                df_b,
                left_on=('pos_a', 'end_a'),
                right_on=('pos_b', 'end_b'),
                how='inner',
                coalesce=False,
            )
            .filter(*self._join_predicates)
            .select(*self._join_cols)
            .filter(*self._join_filters)
            .with_columns(self.weight_strategy.expr.alias('weight'))
            .sort(['index_a', 'index_b'])
        )
